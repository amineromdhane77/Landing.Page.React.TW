import React, { useState } from 'react';
import logo from '../assets/logo.webp'
import { IoMdClose } from "react-icons/io";
import { CiMenuBurger } from "react-icons/ci";
import { LINKS } from '../indexx';
const Navbar = () => {
    const [isOpen,setIsOpen]=useState(false)
    const toggleMenu=()=>{
        setIsOpen(!isOpen)
    }
  return (
    <nav className=' border-b-2'>
      <div className=' max-w-7xl mx-5 flex justify-between items-center py-8'>
      <div className=' pl-2'>
      <a href='#'>
      <img src={logo} width={150} height={25} alt='vastu'/>
      </a>
      </div>
      <div className='md:hidden'>
      <button onClick={toggleMenu} className=' text-2xl pr-2 focus:outline-none' aria-label={isOpen?'close menu' :'open menu'}>
      {isOpen ? <IoMdClose /> :<CiMenuBurger />}
      </button>
      </div>
      <div className=' hidden md:flex space-x-8 md:space-x-7 pr-2'>
      {LINKS.map((link,index)=>(
        <a key={index} href={link.link} className=' uppercase text-sm font-medium'>
        {link.name}
        </a>
      ))}
      </div>
      </div>
      <div className={`${isOpen ? "block" : "hidden"} md:hidden absolute bg-white w-full py-5 px-4 mt-4 border-b-2 `}>
      {LINKS.map((link,index)=>(
        <a key={index} href={link.link} className=' uppercase text-lg font-medium block py-2 tracking-wide'>
        {link.name}
        </a>
      ))}
      </div>
    </nav>
  );
}

export default Navbar;
